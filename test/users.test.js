  ...
  describe('POST /api/users', function () {
    ...
    it('Should reject users with non-string username', function () {
      return chai
        .request(app)
        .post('/api/users')
        .send({username: 42, password, fullname})
        .then(res => {
          expect(res).to.have.status(422);
    ...
    it('Should reject users with non-trimmed username', function () {
      return chai
        ...
        .then(res => {
          expect(res).to.have.status(422);
          expect(res.body.message).to.equal("username cannot start or end with a space.");
        });
    });
    ...
    it('Should reject users with non-trimmed password', function () {
      return chai
        ...
        .then(res => {
          expect(res).to.have.status(422);
          expect(res.body.message).to.equal("password cannot start or end with a space.");
        });
    });
    ...
    it('Should reject users with password less than 8 characters', function () {
      return chai
        .request(app)
        .post('/api/users')
        .send({username, password: 'tusk', fullname})
